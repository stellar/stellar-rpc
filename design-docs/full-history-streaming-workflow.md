...
The catalog holds four groups of keys (including the deployment's cold-index secret under meta/): per-chunk artifact state keys, hot DB state keys, and the config pin.
...
5. **LedgerBackend choice or mid-flight swap.** The LedgerBackend contract guarantees canonical LCM bytes for any range, so any conformant backend produces byte-identical artifacts WITHIN a deployment (cold index routing is keyed by the deployment's persisted catalog secret, so cross-deployment artifact bytes differ by design). Different backends differ in performance, not behavior. An operator using BSB for backfill and CaptiveCore for hot DB ingestion, or swapping mid-deployment, satisfies all four invariants.
