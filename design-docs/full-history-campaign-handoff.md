...
already owns two bench-gated follow-ups (RunReader per-byte decode,
writer-side bloom collection).
...
5. **Review skip-notes worth revisiting** (see commit "review:
   simplify/reuse/efficiency pass"): RunReader per-byte varint decode
   (~5-10s/merge, bench-gated), writer-side bloom/fence collection (saves a
   full re-read per seal/merge, touches trust path), terms.run frame
   generalization, heap unification, ledger.AddLedgerToBatch deletion.
