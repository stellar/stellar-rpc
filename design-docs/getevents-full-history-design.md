...
1. Blind the term key under the chunk secret to get its routed key, and look that up in the directory. A demoted term names the parts its window reaches: `p ∈ [Start >> (16+k), (End−1) >> (16+k)]` clamped to `[0, partCount)`, at items `128·(firstRecord + p)`.
2. Otherwise query the MPHF in `index.hash` with the routed key for the slot; the item is at that position.
3. Read every position in one pass. An item matches when its fingerprint equals streamhash's fingerprint of the routed key (`streamhash.Fingerprint`). A bucket item whose fingerprint disagrees is a miss — the MPHF answers for keys it never saw — but a part item's fingerprint must match, since the directory named the key.
...
* The window exists so a reader can read only the part of a term it is asked for; the covered range exists so it can report the whole of what it read, since a reader whose unit is wider than the window (a whole part) has already paid for the remainder. A reader that returns whole terms satisfies the contract for free and reports the **whole id space**: the hot store does, its images being whole-chunk and already in memory, and so does a cold lookup no demoted term was in, every one of its terms having come out of a whole bucket item. A query over such a lookup runs in one stage — a second would ask for ids the first already answered for.
