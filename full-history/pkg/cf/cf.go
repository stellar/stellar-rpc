// Package cf provides column family (CF) routing for transaction hash partitioning.
//
// Transaction hashes are partitioned into 16 column families based on the
...
const (
	// Count is the number of column families (hash nibble partitions).
	// Each CF gets its own RecSplit index file (cf-{nibble}.idx).
	Count = 16
)

// Names maps CF index (0-15) to the lowercase hex nibble string.
...
// Index returns the CF index (0-15) for a transaction hash.
...
func Index(txHash []byte) int {
	return int(txHash[0] >> 4)
}

// Name returns the CF name (hex nibble "0"-"f") for a transaction hash.
func Name(txHash []byte) string {
	return Names[txHash[0]>>4]
}
