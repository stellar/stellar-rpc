...
# Honor a `replace` directive for the Go SDK (e.g. a fork carrying unreleased XDR).
GO_SDK_REPO=$(go list -m -f '{{if .Replace}}{{.Replace.Path}}{{else}}{{.Path}}{{end}}' github.com/stellar/go-stellar-sdk)
GO_XDR_REVISION=$(go list -m -f '{{if .Replace}}{{.Replace.Version}}{{else}}{{.Version}}{{end}}' github.com/stellar/go-stellar-sdk | $SED 's/.*-\(.*\)/\1/')

# revision of https://github.com/stellar/stellar-xdr/ used by the Go code
STELLAR_XDR_REVISION_FROM_GO=$($CURL https://raw.githubusercontent.com/${GO_SDK_REPO#github.com/}/${GO_XDR_REVISION}/xdr/xdr_commit_generated.txt)
