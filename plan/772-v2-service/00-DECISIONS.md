...
| D10 | v1-parity boundary (added 2026-07-15, resolved with Karthik during the stage-5 session) | v1 is PRIOR ART, not a conformance target: v2 must be correct on its own terms and does NOT reproduce v1 behaviors whose only justification is "v1 did it". This qualifies D2's "handlers reused verbatim" — reuse stops where it would carry a dead behavior forward. First application: `inSuccessfulContractCall` (deprecated in the public API docs; the SDK's `protocol.EventInfo` says `Deprecated: remove in v24`) is DROPPED from v2 getEvents responses. The event veneer does not compute it (the v2 event payload deliberately has no slot for it), and stage 6 MUST register a v2 getEvents response shape WITHOUT the field — while the shared v1 handler and daemon stay untouched (D1). |
