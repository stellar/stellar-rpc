...
- Exact commands (also in the RUNBOOK):
  ...
  - transfer (scp-only boxes): `docker save stellar-rpc-v2:dev | gzip > stellar-rpc-v2.tar.gz`, `scp` it over, then on the box `gunzip -c stellar-rpc-v2.tar.gz | docker load`
  - run (devbox layout: config `~/stellar-rpc-v2/config`, data `/mnt/nvme/rpc-v2-service`): `docker run -d --restart unless-stopped --name stellar-rpc-v2 -v ~/stellar-rpc-v2/config:/config:ro -v /mnt/nvme/rpc-v2-service:/data -p 8000:8000 -p 8001:8001 stellar-rpc-v2:dev full-history --config /config/full-history.toml`; for a GCS lake add `-v ~/.gcloud:/gcloud:ro -e GOOGLE_APPLICATION_CREDENTIALS=/gcloud/<key>.json` (SDK GCS datastore uses bare `storage.NewClient` ⇒ Application Default Credentials). Stop/erase/re-pin cycle + lake enablement are RUNBOOK §7.
- Deviations from the stage doc + why:
