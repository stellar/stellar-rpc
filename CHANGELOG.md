...
### Changed
* `getTransactions` now scans at most 10,000 ledgers per request. On a sparse range a page can come back short — or even empty — while still carrying a `cursor`, so a response shorter than `limit` no longer implies end-of-data: clients should page until the response carries no `cursor` ([#908](https://github.com/stellar/stellar-rpc/pull/908)).
