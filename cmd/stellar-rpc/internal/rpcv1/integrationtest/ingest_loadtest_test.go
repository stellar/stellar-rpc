...
func runIngestPhase(t *testing.T, sqlitePath string, ledgerPaths []string, prof ingestProfile, maxPerFile uint32) {
	...
	sdb, err := sqlitedb.OpenSQLiteDB(sqlitePath)
...
func verifyLedgerRange(t *testing.T, sqlitePath string, startSeq, endSeq, expected uint32) {
	...
	sdb, err := sqlitedb.OpenSQLiteDB(sqlitePath)
	require.NoError(t, err)
	count, lo, hi, err := sqlitedb.NewLedgerReader(sdb).GetLedgerCountInRange(t.Context(), startSeq, endSeq)
	require.NoError(t, sdb.Close())
	require.NoError(t, err)
...
func getLedgerBounds(ctx context.Context, sdb *sqlitedb.DB) (uint32, uint32, error) {
	r, err := sqlitedb.NewLedgerReader(sdb).GetLedgerRange(ctx)
