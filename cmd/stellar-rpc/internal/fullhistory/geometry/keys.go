...
func TxHashIndexKey(idx TxHashIndexID, lo, hi chunk.ID) string {
	if lo > hi {
		panic(fmt.Sprintf("fullhistory: TxHashIndexKey lo %s > hi %s", lo, hi))
	}
...
func ParsePadded(s string) (uint32, error) {
	if len(s) != 8 {
		return 0, fmt.Errorf("fullhistory: %q is not an 8-digit padded id", s)
	}
	n, err := strconv.ParseUint(s, 10, 32)
	if err != nil {
		return 0, fmt.Errorf("fullhistory: %q is not numeric: %w", s, err)
	}
	return uint32(n), nil
}
