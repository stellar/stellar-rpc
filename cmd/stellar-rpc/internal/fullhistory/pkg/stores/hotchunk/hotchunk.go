// Package hotchunk implements decision (a): the per-chunk hot tier is ONE RocksDB
// instance holding the union of every hot data type's column families — the
// ledger CF, the three events CFs, and the 16 nibble-routed txhash CFs — and each
// ledger commits as ONE atomic, synced WriteBatch across ALL of those CFs. A
// ledger is therefore fully present or fully absent; there is a SINGLE per-chunk
// watermark (the max committed ledger seq, authoritative from the ledgers CF's
// last key), with no per-store frontier markers and no min-of-three.
//
// The three typed facades (ledger/txhash/eventstore HotStore) are composed over
// the one shared store via their NewWithStore constructors, keeping their read
// APIs for downstream (#770); their write paths queue Puts into the shared batch
// so one batch spans all CFs and commits once.
package hotchunk
...
// config builds the shared store's rocksdb.Config. Per-CF options come from the
// events facade (ZSTD on DataCF, tuned block sizes); the txhash workload's DB-wide
// + per-CF tuning (block cache, background jobs, WAL cap, bloom, write-buffer) is
// applied via Tuning. Tuning's per-CF fields apply to every CF — a deliberate,
// benign over-application (the ledger/events CFs just gain a bloom + larger write
// buffer); the per-CF compression/block-size overrides keep events distinct.
...
// highest ledger seq durably committed, from the ledgers CF's last key. Under
// decision (a) this one value pins EVERY CF's frontier (events and txhash never
// trail or lead). ok=false on an empty DB.
...
// IngestLedger commits ONE ledger to the shared hot DB as a SINGLE atomic, synced
// WriteBatch across all enabled CFs (decision (a)): extract each enabled type's
// rows, queue them into one rocksdb.BatchWriter, commit once, and only then apply
// the events facade's in-memory mirror/offsets update.
//
// seq is the driver-validated sequence of lcm, a borrowed zero-copy view: every
// extractor copies what it retains (ledger bytes and tx hashes synchronously into
// the batch; events payloads into fresh buffers in the prepare step), so the view
// need not outlive this call.
//
// An idempotent-duplicate events ledger contributes nothing in prepare and a nil
// apply hook; the other (upsert-keyed) CFs still write, matching the merged
