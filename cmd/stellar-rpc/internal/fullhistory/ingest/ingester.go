...
//
// Input: a zero-copy xdr.LedgerCloseMetaView (a []byte alias over the source
// stream's BORROWED buffer). The view is valid only for the current iteration
// step; an ingester must copy any bytes it retains. The hot fan-out (HotService)
// waits for all ingesters to finish a ledger before the source pulls the next
// one, so synchronous consumption inside Ingest is safe.
//
...
// ledgerSeqOf reads the ledger sequence off a borrowed view. It is the single
// place ingesters and services derive the seq from an LCM.
func ledgerSeqOf(lcm xdr.LedgerCloseMetaView) (uint32, error) {
	return lcm.LedgerSequence()
}
