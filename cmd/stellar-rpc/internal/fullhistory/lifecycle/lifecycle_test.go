...
func TestRunLifecycleTick_DiscardGatedOnIndexCoverage(t *testing.T) {
	...
	ops, err := eligibleDiscardOps(cat, floorFor(t, cfg, lastChunk.LastLedger()), lastChunk, nil)
	require.NoError(t, err)
...
func TestRunLifecycleTick_PrunesTransientIndexDebris(t *testing.T) {
	...
	ops, weights, err := eligiblePruneOps(cat, floorFor(t, cfg, geometry.PreGenesisLedger), nil)
	require.NoError(t, err)
