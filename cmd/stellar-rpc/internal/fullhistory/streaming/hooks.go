...
// catalog_protocol.go), not of a test that hand-replays those steps. A
// hand-inlined sweep can stay green even after the production order is broken;
// a hook fired from INSIDE the real method cannot. Each hook observes durable
// state at the exact instant between two steps and lets the test assert the
// invariant that the step ORDER is meant to guarantee:
//
