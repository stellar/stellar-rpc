...
const (
	// defaultRetryBackoff is the base inter-retry delay; maxRetryBackoff caps the
	// exponential growth so a high MaxRetries cannot wait unboundedly.
...
// capped at maxRetryBackoff. A non-positive base falls back to defaultRetryBackoff.
func retryBackoffFor(attempt int, base time.Duration) time.Duration {
	if base <= 0 {
		base = defaultRetryBackoff
	}
	// Guard the shift against int64 overflow for a large MaxRetries: past ~30
	// doublings the delay is already pinned at the cap.
	if attempt > 30 {
...
// ctxSleep is withRetries' production backoff wait: a ctx-aware timer that
// returns nil after d elapses or ctx.Err() if the context is canceled first.
