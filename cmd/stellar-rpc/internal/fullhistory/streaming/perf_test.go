...
func TestStreamingRebuild_ByteIdenticalToColdPath(t *testing.T) {
	...
	require.True(t, ok)
	streamingIdx := cat.layout.IndexFilePath(frozen)
...
func TestStreamingIdx_MatchesSpecFormat(t *testing.T) {
	...
	idx, err := streamhash.OpenPayload(cat.layout.IndexFilePath(frozen))
	require.NoError(t, err)
...
func TestColdIndexSizing_ConsistentWithPart4(t *testing.T) {
	...
	for c := chunk.ID(0); c <= 2; c++ {
		...
		for i := 0; i < perChunk; i++ {
			//nolint:gosec // small test offsets, well within the chunk
	...
	info, err := os.Stat(cat.layout.IndexFilePath(frozen))
	require.NoError(t, err)
	bytesPerTx := float64(info.Size()) / float64(n)
	t.Logf("cold .idx: %d bytes over %d keys = %.3f B/tx (design Part-4 asymptote ≈4.2 B/tx at the dense window)", info.Size(), n, bytesPerTx)
