...
func spillRun(t *testing.T, dir, name string, recs map[events.TermKey][]uint32) string {
	t.Helper()
	slab := NewSlab(4 << 20)
...
func collectMerge(t *testing.T, paths []string) map[events.TermKey][]uint32 {
	t.Helper()
	out := map[events.TermKey][]uint32{}
	var order []events.TermKey
	require.NoError(t, MergeRuns(paths, func(term events.TermKey, ids []uint32) error {
		_, dup := out[term]
...
func TestMergeRuns_UnionsAcrossRuns(t *testing.T) {
	...
	shared := key(1)
	paths := []string{
		spillRun(t, dir, "a.run", map[events.TermKey][]uint32{
			shared: {0, 3, 6}, key(2): {10, 11},
		}),
		spillRun(t, dir, "b.run", map[events.TermKey][]uint32{
			shared: {1, 3, 7}, key(3): {20},
		}),
		spillRun(t, dir, "c.run", map[events.TermKey][]uint32{
			shared: {2, 8}, key(2): {12},
		}),
	}
	got := collectMerge(t, paths)
	assert.Equal(t, map[events.TermKey][]uint32{
...
func TestMergeRuns_MatchesReference(t *testing.T) {
	...
	dir := t.TempDir()
	want := map[events.TermKey][]uint32{}
	paths := make([]string, 0, 12)
	nextID := uint32(0)
	for run := range 12 {
		recs := map[events.TermKey][]uint32{}
		for range 500 {
...
func TestMergeRuns_SingleRunPassThrough(t *testing.T) {
	dir := t.TempDir()
	recs := map[events.TermKey][]uint32{key(5): {1, 2, 3}, key(9): {7}}
...
func TestMergeRuns_EmptyInput(t *testing.T) {
	require.NoError(t, MergeRuns(nil, func(events.TermKey, []uint32) error {
