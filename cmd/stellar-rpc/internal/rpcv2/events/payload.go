...
// assigned identically to the SQLite path (sqlitedb/event.go), so the two backends
// return the same IDs.
//
// Because PayloadShaper emits each ledger in ascending getEvents cursor
...
type Payload struct {
	...
	// (sqlitedb/event.go) so both backends produce the same IDs.
	EventIdx uint32
