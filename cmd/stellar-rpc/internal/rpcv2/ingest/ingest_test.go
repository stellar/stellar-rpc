...
func TestEventsColdWriter_Readback(t *testing.T) {
	...
	bms, _, err := cr.LookupKeys(context.Background(), []event.TermKey{term}, termWindow)
	require.NoError(t, err)
...
func TestWriteColdChunk_EventlessChunk_FullyReadable(t *testing.T) {
	...
	bms, _, lerr := cr.LookupKeys(context.Background(), []event.TermKey{anyKey}, termWindow)
...
func TestColdChunk_Success(t *testing.T) {
	...
	bms, _, err := ecr.LookupKeys(context.Background(), []event.TermKey{term}, termWindow)
	require.NoError(t, err)
	require.Equal(t, uint64(2), bms[0].GetCardinality())
...
func TestColdChunk_MidChunkFailure_NoArtifact(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "extract ledger tx parts", "the shared walk rejects the garbage ledger")
...
func TestWriteColdChunk_ByteIdentity_SharedWalk(t *testing.T) {
	...
	bms, _, err := ecr.LookupKeys(context.Background(), terms, termWindow)
	require.NoError(t, err)
	for i, k := range terms {
