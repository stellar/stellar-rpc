...
func FreezeColdChunk(
	...
	if cfg.Ledgers {
		...
		if err := freezeArm(sink, dataTypeLedgers, func() (int, error) {
			// Concurrency buys hashing, not encoding: PreCompressed copies
			// frames verbatim, but the pack's content hash is over RAW
			// ledger bytes, so every frame is decompressed once on a
			// packfile worker's hash goroutine. That decode is the freeze's
			// only CPU stream, so it gets the same width the raw-mode
			// (walk/backfill) writer encodes with. Writeback smoothing
			// applies as in every batch build. The copy runs at full speed
			// and is CORRECT on any topology; the RECOMMENDED deployment
			// puts cold artifacts on their own device, because on a shared
			// disk an unpaced copy stream queues ahead of the hot WAL
			// fdatasync (measured: ~50ms median fsync waits → co-located
			// ingest commit p50 +28ms for the arm's duration).
	...
	if cfg.Txhash {
		...
		if err := freezeArm(sink, dataTypeTxhash, func() (int, error) {
			return db.FreezeTxhashCold(ctx, dirs.TxhashBin, secret)
		}); err != nil {
			return err
		}
	}
	if cfg.Events {
		...
		if err := freezeArm(sink, dataTypeEvents, func() (int, error) {
			...
			return 0, db.FreezeEventsCold(ctx, scratch, dirs.EventsDir, secret, event.ColdWriterOptions{
				Concurrency:  coldEncoderConcurrency,
				BytesPerSync: coldBytesPerSync,
			})
		}); err != nil {
