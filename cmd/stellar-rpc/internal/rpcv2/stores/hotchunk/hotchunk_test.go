...
func TestIngestLedger_AllCFsAdvanceTogether(t *testing.T) {
	...
	// events CFs.
	bms, _, err := db.Events().LookupKeys(context.Background(), []event.TermKey{termA}, termWindow)
	require.NoError(t, err)
...
func TestIngestLedger_WritesEveryHotType(t *testing.T) {
	...
	bms, _, err := db.Events().LookupKeys(context.Background(), []event.TermKey{term}, termWindow)
	require.NoError(t, err)
	require.NotNil(t, bms[0])
