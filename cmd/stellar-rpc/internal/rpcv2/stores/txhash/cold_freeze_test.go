...
// cross-path identity gate: the freeze-written .bin must be byte-for-byte
// identical to the walk path's accumulate-then-sort output. Both paths blind
// with the same secret and sort by the blinded key, so identity reduces to
// the sort's determinism (keys are unique — no tie-break sensitivity).
func TestFreezeColdFromStore_ByteIdenticalToWalk(t *testing.T) {
	chunkID := chunk.ID(0)
	entries := freezeEntries(chunkID, 5000)

	// Walk path: blind, sort, write — exactly what ingest's finalize does
	// (the stored key is BlindKey(secret, hash[:ColdKeySize])).
	walk := make([]ColdEntry, len(entries))
	for i, e := range entries {
		walk[i].Key = stores.BlindKey(testBinSecret, e.Hash[:ColdKeySize])
	...
	require.NoError(t, WriteColdBin(walkPath, testBinSecret, walk))
	...
	n, err := FreezeColdFromStore(context.Background(), chunkID, store, freezePath, testBinSecret)
...
func TestFreezeColdFromStore_EmptyChunk(t *testing.T) {
	...
	n, err := FreezeColdFromStore(context.Background(), chunk.ID(0), store, freezePath, testBinSecret)
	require.NoError(t, err)
	require.Zero(t, n)

	walkPath := filepath.Join(t.TempDir(), "walk.bin")
	require.NoError(t, WriteColdBin(walkPath, testBinSecret, nil))
	walkBytes, err := os.ReadFile(walkPath)
	require.NoError(t, err)
	freezeBytes, err := os.ReadFile(freezePath)
	require.NoError(t, err)
...
func TestFreezeColdFromStore_RejectsOutOfRangeSeq(t *testing.T) {
	...
	_, err := FreezeColdFromStore(context.Background(), chunkID, store, filepath.Join(t.TempDir(), "x.bin"), testBinSecret)
	require.Error(t, err)
