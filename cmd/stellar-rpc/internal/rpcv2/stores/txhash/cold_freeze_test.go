	...
	for i, hashes := range ledgers {
		for _, h := range hashes {
			entries = append(entries, ColdEntry{
				Key: stores.BlindKey(testBinSecret, h[:ColdKeySize]),
				Seq: first + uint32(i),
			})
		}
	}
...
func freezeToBytes(t *testing.T, chunkID chunk.ID, store *rocksdb.Store, wantEntries int) []byte {
	...
	n, err := FreezeColdFromStore(context.Background(), chunkID, store, binPath, testBinSecret)
	require.NoError(t, err)
...
func TestFreezeAndGet_OuterInnerResolveAcrossTiers(t *testing.T) {
	...
	_, err := FreezeColdFromStore(context.Background(), chunkID, store, binPath, testBinSecret)
	require.NoError(t, err)
