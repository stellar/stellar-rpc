...
// run_reader.go — the ONE streaming reader for TXHRUN02 run files (format
...
// end-of-stream), key sortedness, and per-record seq containment in the
...
// manifest-listed run as one k-way source, copying its records into the
// .bin verbatim.
//
// The reader is content-blind: it never blinds or interprets a key, so a
// wrong secret is invisible here (it is caught at open — hot_store.go).
...
// consumer loudly. Per-record key order is checked as it streams (merge
...
func (r *runSource) key() []byte { return r.rec[:ColdKeySize] }

func (r *runSource) seq() uint32 { return binary.LittleEndian.Uint32(r.rec[ColdKeySize:]) }
...
func (r *runSource) advance() (bool, error) {
	...
	if idx > 0 {
		copy(r.prev[:], r.rec[:ColdKeySize])
	}
	if _, err := io.ReadFull(r.br, r.rec[:]); err != nil {
		return false, fmt.Errorf("run %s: record %d: %w", r.path, idx, err)
	}
	_, _ = r.crc.Write(r.rec[:])
	if idx > 0 && bytes.Compare(r.prev[:], r.rec[:ColdKeySize]) > 0 {
		return false, fmt.Errorf("run %s: record %d out of key order", r.path, idx)
	}
