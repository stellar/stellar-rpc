...
//
// Materializing does not retain p.ids, and it must not: for a sparse term
// those are the store's LIVE published ids (ConcurrentBitmaps.Get hands the
// slice straight out). roaring v2.18.2's BitmapOf is NewBitmap followed by
// AddMany, and AddMany walks the slice copying each value into a container —
// the bitmap keeps no reference to the argument. That is why there is no
// defensive clone here: it would allocate a second copy of every sparse
// term's ids for nothing.
