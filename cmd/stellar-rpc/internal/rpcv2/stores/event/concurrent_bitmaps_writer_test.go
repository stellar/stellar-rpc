...
// TestConcurrentBitmaps_SnapshotSurvivesMultiContainerWrites pins the
// core safety property of the writer-owned dense bitmap: a snapshot
// handed to a reader is never mutated afterwards, even when the
// writer keeps adding ids in place across many roaring containers.
//
// The ids deliberately span ~16 containers (65_536 ids each) so the
// writer exercises both getWritableContainerAtIndex (COW deep copy of
// a container the snapshot shares) and insertNewKeyValueAt (a brand
// new container the snapshot never saw).
...
// TestConcurrentBitmaps_PublishBeforeClearingDirty pins the two
// publisher-side ordering rules that denseState.snapshot's freshness
// argument rests on, deterministically:
//
//   - AddTo flags the term dirty before it returns, and leaves the
//     published snapshot alone;
//   - a republish stores pub before clearing dirty, so dirty==false
//     always implies pub already holds that snapshot.
//
// The reader-side half of the argument (load dirty, then pub) cannot
// be pinned by a test: breaking it opens a window only two
// instructions wide, which stress testing does not reach. It is
// argued in the comment on denseState.snapshot instead.
func TestConcurrentBitmaps_PublishBeforeClearingDirty(t *testing.T) {
	s := newTestConcurrentBitmaps()
	...
	s.AddTo(key, 12_345)
	assert.True(t, d.dirty.Load(), "AddTo must flag dirty before returning")
	assert.Same(t, first, d.pub.Load(), "AddTo must not touch the published snapshot")
...
// TestConcurrentBitmaps_FreshnessUnderConcurrentPublishers is the
// stress test for the flag-before-pointer ordering in
// denseState.snapshot. The writer publishes the highest ID it has
// finished adding; every reader loads that ID and then calls Get, so
// each Get provably starts after the AddTo that produced the ID
// returned and must therefore observe it.
//
// The failure this catches is not a data race (both sides are
// atomics) but a lost update: if snapshot loaded pub before dirty, a
// reader could take a stale pub, have another reader republish and
// clear dirty in between, then see dirty==false and return the stale
// snapshot. Many readers plus a wide bitmap (a slow Clone) widen that
// window.
func TestConcurrentBitmaps_FreshnessUnderConcurrentPublishers(t *testing.T) {
	...
	// Seed ~600 containers so each republish Clone takes long enough
	// for a concurrent reader to interleave with it.
	seed := make([]uint32, 0, 600*8)
	for c := range uint32(600) {
	...
	const runFor = 750 * time.Millisecond
	...
	for range numReaders {
		wg.Go(func() {
			for !done.Load() {
				...
				reads.Add(1)
				// Get started strictly after the AddTo that committed
				// `want` returned, so the snapshot must contain it.
				if !bm.Contains(want) {
	...
	wg.Wait()
	t.Logf("freshness stress: %d reads against %d committed ids",
		reads.Load(), committed.Load())
