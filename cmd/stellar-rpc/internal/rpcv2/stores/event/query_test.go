...
func TestQueryPage_MatchAllPagesToExhaustion(t *testing.T) {
	fx := newQueryFixture(t)
	rng := wholeChunk(t, fx.store)
	var all []string
	pos := rng.Start
	for pages := 0; ; pages++ {
		require.Less(t, pages, 5, "paging must terminate")
		page, err := QueryPage(context.Background(), fx.store, nil,
			QueryOptions{Range: IDRange{Start: pos, End: rng.End}, MaxEvents: 2})
		require.NoError(t, err)
		require.LessOrEqual(t, len(page.Payloads), 2)
		all = append(all, dataSyms(t, page.Payloads)...)
		if page.Exhausted {
			assert.Equal(t, rng.End, page.NextStart, "an exhausted page resumes at the window end")
			break
		}
		require.Greater(t, page.NextStart, pos, "pages must advance")
		pos = page.NextStart
	}
	assert.Equal(t,
		[]string{"evt-a-ab", "evt-a-ac", "evt-b-ab", "evt-b-a", "evt-a-b"}, all,
		"pages must cover the window exactly once, in order")
}

func TestQueryPage_FilteredPagesToExhaustion(t *testing.T) {
	fx := newQueryFixture(t)
	rng := wholeChunk(t, fx.store)
	filters := []Filter{{ContractID: fx.contractA[:]}} // ids 0, 1, 4

	page1, err := QueryPage(context.Background(), fx.store, filters,
		QueryOptions{Range: rng, MaxEvents: 2})
	require.NoError(t, err)
	assert.Equal(t, []string{"evt-a-ab", "evt-a-ac"}, dataSyms(t, page1.Payloads))
	require.False(t, page1.Exhausted, "a truncated selection is not exhaustion")
	assert.Equal(t, uint32(2), page1.NextStart, "one past the last selected id")

	page2, err := QueryPage(context.Background(), fx.store, filters,
		QueryOptions{Range: IDRange{Start: page1.NextStart, End: rng.End}, MaxEvents: 2})
	require.NoError(t, err)
	assert.Equal(t, []string{"evt-a-b"}, dataSyms(t, page2.Payloads))
	assert.True(t, page2.Exhausted)
}

func TestQueryPage_UncappedAndExactCapAreOnePage(t *testing.T) {
	fx := newQueryFixture(t)
	rng := wholeChunk(t, fx.store)
	for _, maxEvents := range []int{0, 5} {
		page, err := QueryPage(context.Background(), fx.store, nil,
			QueryOptions{Range: rng, MaxEvents: maxEvents})
		require.NoError(t, err)
		assert.Len(t, page.Payloads, 5)
		assert.True(t, page.Exhausted, "MaxEvents=%d covers the whole window", maxEvents)
		assert.Equal(t, rng.End, page.NextStart)
	}
}

func TestQueryPage_EmptyWindowIsExhausted(t *testing.T) {
	fx := newQueryFixture(t)
	page, err := QueryPage(context.Background(), fx.store, nil,
		QueryOptions{Range: IDRange{Start: 2, End: 2}})
	require.NoError(t, err)
	assert.Empty(t, page.Payloads)
	assert.True(t, page.Exhausted)
	assert.Equal(t, uint32(2), page.NextStart)
}

func TestQueryPage_NoMatchesIsExhausted(t *testing.T) {
	fx := newQueryFixture(t)
	var unknown xdr.ContractId
	unknown[0] = 0x99
	page, err := QueryPage(context.Background(), fx.store,
		[]Filter{{ContractID: unknown[:]}},
		QueryOptions{Range: wholeChunk(t, fx.store), MaxEvents: 2})
	require.NoError(t, err)
	assert.Empty(t, page.Payloads)
	assert.True(t, page.Exhausted, "no matches at all: nothing left to page through")
}

func TestQueryPage_DescendingRejected(t *testing.T) {
	fx := newQueryFixture(t)
	_, err := QueryPage(context.Background(), fx.store, nil,
