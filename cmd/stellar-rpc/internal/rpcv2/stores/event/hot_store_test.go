...
func TestHotStore_PostCloseReadsError(t *testing.T) {
	...
	bms, err := h.store.LookupKeys(context.Background(), []TermKey{keys[0]}, everyID)
	assert.Nil(t, bms)
...
func TestHotStore_ConcurrentIngestAndLookup(t *testing.T) {
	...
	go func() {
		...
		for range N {
			...
			if _, err := h.store.LookupKeys(context.Background(), keys[:1], everyID); err != nil {
...
func lookupOne(t *testing.T, r Reader, key TermKey) *roaring.Bitmap {
	t.Helper()
	bms, err := r.LookupKeys(context.Background(), []TermKey{key}, everyID)
	require.NoError(t, err)
	require.Len(t, bms, 1)
	return bms[0]
}
