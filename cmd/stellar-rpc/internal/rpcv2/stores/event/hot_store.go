...
func (h *HotStore) applyLedger(perKeyIDs map[TermKey][]uint32, eventCount uint32) {
	...
	// key takes one AddTo per ledger instead of one per event — saving the
	// per-term lock round-trips, and the grouping work itself happens once
	// per ledger instead of twice.
...
//
// Implementation: build into a single-threaded Bitmaps, then convert to
// ConcurrentBitmaps at the end — this keeps warmup off the concurrent
// index's per-term locking and promotion path, one AddTo per term instead
// of one per index row (~50M for a 10M-event chunk). Packed rows arrive
// pre-grouped per term; legacy rows are grouped via the prevTerm/buf
// accumulator (byte-sorted iteration delivers a term's legacy rows
// consecutively).
//
