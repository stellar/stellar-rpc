...
func (h *HotStore) applyLedger(perKeyIDs map[events.TermKey][]uint32, eventCount uint32) {
...
// fresh events.ConcurrentBitmaps. Design doc §12 step 3. Two row formats
...
//
// Implementation: build into a single-threaded events.Bitmaps, then convert
...
func warmupIndex(chunkStore *rocksdb.Store) (*events.ConcurrentBitmaps, uint64, error) {
	...
	for entry, err := range chunkStore.Iterate(IndexCF, nil) {
		...
		case packedIndexKeyLen:
			...
			flush()
			if derr := decodePackedIndexRow(entry.Value, func(term events.TermKey, ids []uint32) {
