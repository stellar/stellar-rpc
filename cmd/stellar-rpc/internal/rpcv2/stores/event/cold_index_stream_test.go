...
// TestWriteColdIndexFromRuns_EmptyChunkReadsBack opens an EVENTLESS chunk's
// streamed artifacts through the production reader. The byte comparison in
// TestWriteColdIndexFromRuns_EmptyChunk pins the bytes but cannot pin the
// read: a zero-term index takes a branch of its own in OpenColdReader — the
// empty index is cross-checked against events.pack's count, and no MPHF
// routing ever runs — so the streamed build's eventless output reaches the
// reader through code the eventful read-back test never touches.
func TestWriteColdIndexFromRuns_EmptyChunkReadsBack(t *testing.T) {
	const chunkID = chunk.ID(9)
	dir := t.TempDir()
	require.NoError(t, WriteColdIndexFromRuns(
		context.Background(), chunkID, nil, t.TempDir(), dir, testIndexSecret))
	writeEventsPackForRuns(t, chunkID, dir, 0)
	...
	require.NoError(t, err)
	assert.Zero(t, cnt, "an eventless chunk's pack holds no events")

	// Every term misses cleanly: absent postings and no error, rather than a
	// routing failure against an index with nothing to route to.
	absent := ComputeTermKey([]byte("never-spilled"), FieldTopic1)
	got, err := cr.LookupKeys(context.Background(), []TermKey{absent})
	require.NoError(t, err)
	require.Len(t, got, 1)
	assert.False(t, got[0].Present(), "an eventless chunk resolves every term to a miss")
	assert.Zero(t, got[0].Cardinality())
	assert.Nil(t, lookupOne(t, cr, absent), "the shared single-term path must agree")
}
