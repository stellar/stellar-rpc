...
// (the multi-MiB records the former 64MiB byte cap falsely aborted on)
...
func buildSortedHash(ctx context.Context, termsRunPath, indexHashPath string, total uint64) error {
	...
	defer f.Close()
	var fed int
	if err := streamTermsRun(f, func(term events.TermKey, _ []byte, _ int64) error {
...
// re-hashed at emit.
func streamTermsRun(f *os.File, emit func(term events.TermKey, body []byte, bodyOff int64) error) error {
...
func writeSlotOrdered(pw *packfile.Writer, termsRunPath string, m *mphf) error {
	...
	err = streamTermsRun(termsRun, func(term events.TermKey, body []byte, bodyOff int64) error {
