...
func WriteColdIndexFromRuns(
	...
	pw, err := packfile.Create(indexPackPath, packfile.WriterOptions{
		...
		ContentHash:    true,
		Overwrite:      true,
		// index.pack's on-disk identity, shared with the in-memory builder:
		// the two emitters write the same artifact, so every option that
		// reaches the bytes has to match or the byte-identity gate fails —
		// and an unchecked index.pack is refused at open.
	...
	// that reports success. The build stamp is byte-identical to the
	// in-memory builder's — same compiled constants — so the two emitters'
	// packs stay comparable byte for byte.
	if err = pw.Finish(encodeIndexBuildStamp()); err != nil {
