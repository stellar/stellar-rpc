...
// encoding (AppendTermPostings). Runs are scratch — non-durable,
...
func (s *Slab) Append(term [16]byte, id uint32) bool {
...
func (s *Slab) EmitSorted(emit func(term [16]byte, ids []uint32) error) error {
...
func (rw *RunWriter) Append(term [16]byte, ids []uint32) error {
	rw.buf = AppendTermPostings(rw.buf[:0], term, ids)
	rw.records++
...
func (r *RunReader) Next() ([16]byte, []uint32, error) {
	var term [16]byte
	if r.remain == 0 {
	...
	// deltas, uint32 overflow) is the shared DecodeAscendingIDs core —
	...
	ids, err := DecodeAscendingIDs(r.uvarint, count, r.ids[:0])
	if err != nil {
