...
func TestColdReader_EventlessChunk(t *testing.T) {
	...
	bms, _, err := cr.LookupKeys(context.Background(), []TermKey{someTerm}, everyID)
	require.NoError(t, err)
	require.Len(t, bms, 1)
	assert.Nil(t, bms[0])
...
func TestColdReader_EmptyIndexOverNonEmptyPackErrors(t *testing.T) {
	...
	_, _, lerr := cr.LookupKeys(context.Background(), []TermKey{contractTermKey(payloads[0])}, everyID)
	require.Error(t, lerr, "a mispaired empty index must error, not miss silently (nil bitmap)")
	// Since the directory carries the key count, the pairing check catches
	// this one before the eventless cross-check gets a chance to.
...
func TestColdReader_OpenMissingIndexHash(t *testing.T) {
	...
	_, _, err = cr.LookupKeys(context.Background(), []TermKey{{}}, everyID)
...
func TestColdReader_PostCloseMethodsError(t *testing.T) {
	...
	_, _, err = cr.LookupKeys(context.Background(), []TermKey{contractTermKey(payloads[0])}, everyID)
...
func TestColdReader_LookupKeys(t *testing.T) {
	...
	bms, covered, err := cr.LookupKeys(context.Background(), keys, everyID)
	require.NoError(t, err)
	require.Len(t, bms, len(keys))
	assert.Equal(t, IDRange{End: math.MaxUint32}, covered, "whole terms cover the whole id space")
...
func TestColdReader_LookupKeysClonesAcrossCalls(t *testing.T) {
	...
	key := contractTermKey(payloads[0])
	first, _, err := cr.LookupKeys(context.Background(), []TermKey{key}, everyID)
	require.NoError(t, err)
	require.Len(t, first, 1)
	require.NotNil(t, first[0])
	first[0].Add(999_999)

	second, _, err := cr.LookupKeys(context.Background(), []TermKey{key}, everyID)
	require.NoError(t, err)
...
func TestColdReader_RejectsMispairedIndexHash(t *testing.T) {
	...
	_, _, err = cr.LookupKeys(context.Background(), []TermKey{contractTermKey(payloadsA[0])}, everyID)
...
func rewriteIndexPackStamp(t *testing.T, dir string, chunkID chunk.ID, schema uint16, mask uint64) {
	t.Helper()
	// The stamp gate runs on the whole app-data blob, so it needs the
	// directory header behind it — all zeros here, which is what an index
	// over no terms carries.
	stamp := make([]byte, indexStampLen+indexDirHeaderLen)
	...
	for _, eventsPerLedger := range []int{2, 0} {
		for _, tc := range cases {
			t.Run(fmt.Sprintf("eventsPerLedger=%d/%s", eventsPerLedger, tc.name), func(t *testing.T) {
				...
				require.NoError(t, err)
				t.Cleanup(func() { _ = cr.Close() })
				_, _, err = cr.LookupKeys(context.Background(), []TermKey{{1}}, everyID)
...
func firstDemoted(t *testing.T, f *partsFixture, d indexDirectory) (string, partEntry) {
	...
	for candidate := range f.oracle {
		e, demoted := d.lookupRouted(routedKey(testIndexSecret, f.key(candidate)))
...
func partsDirectoryCorruptions() []partsCorruption {
	return []partsCorruption{
		...
		},
		{
			...
			corrupt: func(t *testing.T, dir string, _ *partsFixture, _ indexDirectory) string {
				rewriteIndexPack(t, dir, func(a *indexArtifact) { a.format = 0xFE1E000D })
				return denseTerm
			},
