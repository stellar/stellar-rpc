...
func testLimits() QueryEventsLimits {
	return QueryEventsLimits{
		Limits: Limits{
			MaxLimit:     protocol.QueryEventsMaxLimit,
			DefaultLimit: 100,
		},
		TermBudget: protocol.QueryEventsDefaultTermBudget,
	}
}
...
func TestQueryEvents_AscendingPagesToTheTip(t *testing.T) {
	ctx, first := seedView(t)

	page1, err := queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
		MinLedger: first,
		Limit:     new(uint(2)),
	})
	require.NoError(t, err)
	...
	page2, err := queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
		Cursor: page1.Cursor,
	})
	require.NoError(t, err)
...
func TestQueryEvents_ScannedLedgerOnAPageThatCoveredNothing(t *testing.T) {
	t.Run("ascending", func(t *testing.T) {
		ctx, first := seedView(t)
		one := uint(1)
		resp, err := queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
			MinLedger: first,
			Limit:     &one,
		})
		require.NoError(t, err)
...
func TestQueryEvents_RangeBelowGenesisIsAnEmptyCompletePage(t *testing.T) {
	for name, req := range map[string]protocol.QueryEventsRequest{
		"ascending":  {MinLedger: 1, MaxLedger: 1},
		"descending": {MaxLedger: 1, Order: protocol.OrderDescending},
	} {
		t.Run(name, func(t *testing.T) {
			ctx, _ := seedView(t)
			resp, err := queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &req)
			require.NoError(t, err)
...
func TestQueryEvents_ClosedRangeCompletesWithoutCursor(t *testing.T) {
	ctx, first := seedView(t)
...
func TestQueryEvents_MinLedgerBelowGenesisFollowsTheFloorRules(t *testing.T) {
	t.Run("ascending is out of range", func(t *testing.T) {
		ctx, first := seedView(t)
		_, err := queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
			MinLedger: 1,
			MaxLedger: first + 2,
		})
		require.Error(t, err)
...
func TestQueryEvents_TermBudgetRejectsBothRequestShapes(t *testing.T) {
	// Three distinct topic values, so three distinct index terms.
	filters := make([]protocol.QueryEventsFilter, 0, 3)
	for _, label := range []string{"a0", "a1", "b0"} {
		_, raw := symbolScVal(t, label)
		filters = append(filters, protocol.QueryEventsFilter{Topic0: requestTopic(t, raw)})
	}
	limits := testLimits()
	limits.TermBudget = 2
	...
	t.Run("range request", func(t *testing.T) {
		ctx, first := seedView(t)
		_, err := queryEvents(ctx, limits, rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
			MinLedger: first,
			Filters:   filters,
		})
		require.Error(t, err)
	...
	t.Run("cursor request", func(t *testing.T) {
		ctx, first := seedView(t)
		scope, err := eventScope(&protocol.QueryEventsRequest{
			MinLedger: first,
			Filters:   filters,
		}, testOldest, first+2)
		require.NoError(t, err)
		...
		_, err = queryEvents(ctx, limits, rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{Cursor: token})
		require.Error(t, err)
		var data protocol.InvalidParamsErrorData
		requireErrorData(t, err, protocol.ErrorReasonInvalidParams, &data)
		assert.Equal(t, uint32(3), data.TermsUsed)
...
func TestQueryEvents_CursorWithV2ForbiddenFilterIsMalformed(t *testing.T) {
	...
	for name, filter := range map[string]event.Filter{
		...
		t.Run(name, func(t *testing.T) {
			...
			_, err = queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{Cursor: token})
			require.Error(t, err)
...
func TestQueryEvents_MalformedCursorReportsTheServedRange(t *testing.T) {
	ctx, first := seedView(t)
...
func TestQueryEvents_JSONInputFormatIsRejected(t *testing.T) {
	ctx, first := seedView(t)
	...
	_, err := queryEvents(ctx, testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
		...
		Filters:        []protocol.QueryEventsFilter{{Topic0: requestTopic(t, raw)}},
	})
	require.Error(t, err)
	requireErrorData(t, err, protocol.ErrorReasonInvalidParams, nil)
}

func TestQueryEvents_InvalidRequestIsRejectedBeforeAnyRead(t *testing.T) {
	ctx, first := seedView(t)
...
func TestQueryEvents_WithoutAViewIsAnError(t *testing.T) {
	_, err := queryEvents(context.Background(), testLimits(), rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
...
func TestQueryEvents_LimitOverTheOperatorCapIsRejected(t *testing.T) {
	limits := testLimits()
	limits.MaxLimit = 2

	// The second case is over protocol.QueryEventsMaxLimit too, and that check must
	// not answer first.
	for _, over := range []uint{3, protocol.QueryEventsMaxLimit + 1} {
	...
	t.Run("at the cap", func(t *testing.T) {
		ctx, first := seedView(t)
		resp, err := queryEvents(ctx, limits, rpcv2test.SilentLogger(), &protocol.QueryEventsRequest{
			MinLedger: first,
			Limit:     new(uint(2)),
		})
		require.NoError(t, err)
