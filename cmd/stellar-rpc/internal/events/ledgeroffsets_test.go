...
func TestLedgerOffsets_Basic(t *testing.T) {
	...
	assert.EqualValues(t, 4529, m.TotalEvents())
	assert.EqualValues(t, 100, m.StartLedger())
	assert.EqualValues(t, 103, m.EndLedger())
}
...
func TestLedgerOffsets_EventIDRange_FullRange(t *testing.T) {
	...
	assert.EqualValues(t, 4529, end)
}

func TestLedgerOffsets_EventIDRange_MultiLedger(t *testing.T) {
	m := NewLedgerOffsets(100)
	m.Append(100, 1042) // ledger 100: IDs 0–1041
	m.Append(101, 2029) // ledger 101: IDs 1042–2028
	m.Append(102, 4529) // ledger 102: IDs 2029–4528
	m.Append(103, 4529) // ledger 103: 0 events
	m.Append(104, 5000) // ledger 104: IDs 4529–4999

	// Middle two ledgers (101-102).
	start, end, err := m.EventIDRange(101, 102)
	require.NoError(t, err)
	assert.EqualValues(t, 1042, start)
	assert.EqualValues(t, 4529, end)

	// Range spanning a zero-event ledger (102-104).
	start, end, err = m.EventIDRange(102, 104)
	require.NoError(t, err)
	assert.EqualValues(t, 2029, start)
	assert.EqualValues(t, 5000, end)

	// First two ledgers (100-101).
	start, end, err = m.EventIDRange(100, 101)
	require.NoError(t, err)
	assert.EqualValues(t, 0, start)
	assert.EqualValues(t, 2029, end)

	// Last two ledgers (103-104), 103 has 0 events.
	start, end, err = m.EventIDRange(103, 104)
	require.NoError(t, err)
	assert.EqualValues(t, 4529, start)
	assert.EqualValues(t, 5000, end)
}

func TestLedgerOffsets_EventIDRange_SingleLedger(t *testing.T) {
	m := NewLedgerOffsets(100)
	m.Append(100, 0)    // ledger 100: 0 events
	m.Append(101, 2029) // ledger 101: IDs 0–2028
	m.Append(102, 2029) // ledger 102: 0 events
	m.Append(103, 4529) // ledger 103: IDs 2029–4528
...
func TestLedgerOffsets_EventIDRange_LedgerWithZeroEvents(t *testing.T) {
	m := NewLedgerOffsets(100)
	m.Append(100, 500) // ledger 100: 500 events
	m.Append(101, 500) // ledger 101: 0 events
	m.Append(102, 800) // ledger 102: 300 events

	// Zero-event ledger alone — returns empty range, no error.
	start, end, err := m.EventIDRange(101, 101)
	require.NoError(t, err)
	assert.EqualValues(t, start, end)

	// Across the zero-event ledger.
	start, end, err = m.EventIDRange(100, 102)
	require.NoError(t, err)
	assert.EqualValues(t, 0, start)
	assert.EqualValues(t, 800, end)
}

func TestLedgerOffsets_TotalEvents_Empty(t *testing.T) {
	m := NewLedgerOffsets(100)
	assert.EqualValues(t, 0, m.TotalEvents())
}
...
func TestLedgerOffsets_ConcurrentReadWrite(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		for i := uint32(0); i < numLedgers; i++ {
			m.Append(i, (i+1)*eventsPerLedger)
		}
	}()
	...
	assert.EqualValues(t, numLedgers*eventsPerLedger, m.TotalEvents())
}
