	...
	fetchFromDatastore := func(start, end uint32) error {
		...
		for _, ledger := range ledgers {
			if err := addToResult([]db.LedgerMetadataChunk{
				{Lcm: ledger, Header: ledger.LedgerHeaderHistoryEntry()},
			}); err != nil {
				return err
			}
		}
...
func parseLedgerInfo(ledger db.LedgerMetadataChunk, format string) (protocol.LedgerInfo, error) {
	ledgerInfo := protocol.LedgerInfo{
		Hash:            ledger.Header.Hash.HexString(),
		Sequence:        uint32(ledger.Header.Header.LedgerSeq),
		LedgerCloseTime: int64(ledger.Header.Header.ScpValue.CloseTime),
	}

	// In the datastore fetch case, we get the LCM structure rather than DB
	// bytes, so we need to encode it here to unify the paths.
	if ledger.RawLcm == nil {
		var err error
		ledger.RawLcm, err = ledger.Lcm.MarshalBinary()
		if err != nil {
			return protocol.LedgerInfo{}, fmt.Errorf("error marshaling ledger metadata: %w", err)
		}
	}
	...
	default:
		...
		ledgerInfo.LedgerMetadata = base64.StdEncoding.EncodeToString(ledger.RawLcm)
