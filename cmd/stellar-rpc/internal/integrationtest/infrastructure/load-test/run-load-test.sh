...
# Hard self-terminate ceiling, independent of the GHA runner. If the runner is
# force-cancelled or crashes it skips its Terminate step, which would otherwise
# leak this c5.2xlarge + 500GB volume until someone notices. This poweroff is
# anchored to instance boot, not the runner, and (paired with the launch's
# --instance-initiated-shutdown-behavior=terminate) terminates the box. The TTL
# sits above the workflow's 225min job timeout so it never pre-empts a healthy run.
SELF_TERMINATE_MINUTES="${SELF_TERMINATE_MINUTES:-270}"
